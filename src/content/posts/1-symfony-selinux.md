...
---
This repository contains a short introduction to using SELinux to confine
web applications on your server.

The introduction is in the form of a short and interactive tutorial, which
you can go through by getting the `Vagrantfile` in this repository and creating
a VM. If you don't want to run through the steps yourself, you can provision
the VM with the included scripts which set up an environment identical
to the one I work with throughout this tutorial.

### *Why* do I need SELinux?

SELinux is a great tool for securing a Linux system. It enforces _Mandatory
Access Control_ based on a subjects assigned security context from a centrally
controlled security policy. This is in contrast to _Discretionary Access Control_ 
which enforces based on the individual users decisions.
...
application to confine! We'll be setting up a simple nginx / php-fpm stack.
...
which has its own maintained SELinux policy. This repository includes a 
`Vagrantfile` for setting up a CentOS 7 box so I'll be using that as my
testbed.
...
setenforce 0
```
__don't worry, we're going to `setenforce 1` in a bit__
...
configuration here. There is also policy which targets `apache` if you prefer
that web server.
...
so set up a new user named `www` and give it the /sbin/nologin shell.
...
user and group, so let's update the fpm configuration with our new
user.
...
Additionally we're turning on error reporting, so we can see
in our browser what's causing the web server to issue an error page
and also set a valid timezone to supress the annoying PHP warning.
...
Great! We have a working `php-fpm` configuration now, ready to use
as soon as we have a vhost for Symfony to run under.

The next step is creating an nginx configuration file which sets up
the vhost that we're going to be serving Symfony on. Their documentation provides a good example we can use to get started:
...
This is all we need to get Symfony up and running! Enable and start php-fpm + nginx,
and fix permissions on the www directory:
...
Now it's time to put SELinux back into enforcing mode, and see what we have to deal with.
...
The first step here is to look at what SELinux is preventing
...
labeled with the file context `var_t`. This isn't what we want for our
web applications files! Though this seems like a pretty common
...
That gets by the first set of errors, but this is now where it 
starts to get a little bit interesting. If we rm the cache and 
...
`httpd_t` domain write to the cache we'll have to start working on a bit
of policy. Fortunately we already have a file context defined for writable
httpd content in the
(upstream policy)[https://github.com/fedora-selinux/selinux-policy/blob/rawhide-contrib/apache.te#L411].
...
Back in business! Further improvements can be made, though.
If we weren't using php-fpm (which runs everything in the 
`httpd_t` domain) and using Apache instead we'd be breaking
the principle of least privilege. If by some popular demand
people would like this to be updated for Apache then I'll get
around to it.
...
This problem comes from the fact that by default SELinux policy
prevents httpd's from connecting to anywhere on the network (the
...
The `-P` flag we pass tells SELinux to persist this change so it
is applied whenever policy reloads or the machine is rebooted.

#### This doesn't work with PHP7!

PHP7 came with some pretty huge engine improvements, along with better
...
If you have any questions or want to chat about this post
don't hesitate to ping me on Freenode IRC under the nick 'sfix'
or via e-mail at gary.tierney [at] gmx.com.